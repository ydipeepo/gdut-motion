class_name Graph2D extends ColorRect
...
@export var value_x_color := Color.RED
@export var value_y_color := Color.GREEN
...
func _draw() -> void:
	...
	var value_y_polyline: PackedVector2Array
	...
	for index in count:
		...
		var value_y := roundf(remap(value.y, min_level, max_level, size.y, 0.0))
		value_x_polyline.push_back(Vector2(index, value_x))
		value_y_polyline.push_back(Vector2(index, value_y))
		if value.marker:
	...
	if 2 <= value_x_polyline.size():
		draw_polyline(value_x_polyline, value_x_color, 1.0)
	if 2 <= value_y_polyline.size():
		draw_polyline(value_y_polyline, value_y_color, 1.0)
