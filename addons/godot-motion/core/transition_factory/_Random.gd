...
const MIN_SEGMENTS := RandomEasing.MIN_SEGMENTS
const DEFAULT_SEGMENTS := RandomEasing.DEFAULT_SEGMENTS

const MIN_INTENSITY := RandomEasing.MIN_INTENSITY
const MAX_INTENSITY := RandomEasing.MAX_INTENSITY
const DEFAULT_INTENSITY := RandomEasing.DEFAULT_INTENSITY

const DEFAULT_SEED := RandomEasing.DEFAULT_SEED
...
func set_intensity(value: float) -> void:
	if value < MIN_INTENSITY or MAX_INTENSITY < value:
		_AUTOLOAD_CLASS.print_warning(
			&"BAD_INTENSITY",
			get_expression_name(),
			value,
			MIN_INTENSITY,
			MAX_INTENSITY)
		value = clampf(value, MIN_INTENSITY, MAX_INTENSITY)
	_intensity = value
