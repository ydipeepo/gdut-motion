class_name GDUT_Transform3DMotionVelocity extends GDUT_MotionVelocity
...
static func _can_convert_from_array(array: Array, array_size: int) -> bool:
	match array.get_typed_builtin():
		TYPE_NIL:
			if array_size != 1 or 12 < array.size():
				return false
			var valid := true
			var write := 0
			for value: Variant in array:
				match typeof(value):
					...
					_:
						valid = false
						break
			if not valid or write != 12:
				return false
		TYPE_INT, \
		TYPE_FLOAT:
			if array_size != 1 or array.size() != 12:
				return false
		TYPE_VECTOR3, \
		TYPE_VECTOR3I:
...
static func _can_convert_from_packed_int32_array(array: PackedInt32Array, array_size: int) -> bool:
	return array_size == 1 and array.size() == 12
